...
import pyembroidery
imporfrom fastapi import FastAPI, UploadFile, File
from fastapi.responses import JSONResponse
from fastapi.middleware.cors import CORSMiddleware
import pyembroidery
import cv2
...
async def digitize_image(file: UploadFile = File(...)):
    ...
    try:
        ...
        # 2. Adaptive Thresholding (Finds shapes regardless of light/dark)
        # This is better than Canny for solid logos like stars
        ...
        for contour in contours:
            ...
            # Skip the outer frame (anything larger than 95% of image)
            if rw > w * 0.95 or rh > h * 0.95:
                continue
                
            # LOWERED THRESHOLD: Only skip truly tiny noise (less than 10 pixels)
        ...
        # If we didn't find anything, let's at least add a tiny placeholder so it's not '1'
        if found_shapes == 0:
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="0.0.0.0", port=8000) cv2
import numpy as np
import os
import uuid
import base64
